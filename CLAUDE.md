...
- **[CSS設計アーキテクチャ](#🎨-css設計アーキテクチャ重要)** - base.css統合基盤 + results.css/training.css構造
...
- ❌ **system.css使用禁止**（全機能はbase.cssに統合済み）
...
スタイル追加前 → 「UIカタログとbase.cssを確認しましたか？」
```
...
### **⚠️ 重要：system.css使用禁止**
- **system.css**: 現在使用禁止（全機能はbase.cssに統合済み）
- **base.css**: 全ての基盤CSS変数・ユーティリティクラスを含む統合ファイル
- **必須**: 新規実装時はbase.cssのみを参照すること

### **CSS階層構造（最新版）**
```
base.css (統合基盤CSS)
├── CSS変数システム（カラー、スペーシング、境界線等）
├── ベーススタイル（body, html）
├── レイアウトユーティリティ（.flex, .gap-3等）
├── タイポグラフィ（.heading-md等）
├── カラークラス
├── 共通コンポーネント（Glass Card、プログレスバー等）
└── アニメーション
...
### **CSS作業の原則**
1. **base.css**: 全ての基盤CSS（変数・ユーティリティクラス・共通コンポーネント）
2. **results.css/training.css**: ページ固有スタイル（base.cssを継承）
3. **ui-catalog.css**: 独立したカタログ専用スタイル
4. **分離設計**: カタログスタイルは実装に影響しない
5. **system.css**: 使用禁止（全機能はbase.cssに統合済み）

### **作業時の注意点**
- UIカタログでの視覚確認はui-catalog.css内で実装
- 実際のアプリスタイルはbase.css/results.css/training.cssで管理
- **base.cssの変数・ユーティリティクラスを最大活用**
- 重複コードを避けクリーンなCSS構造を維持
- system.cssは参照しない（全てbase.cssに統合済み）

### **CSS調査・修正の正しい手順**
1. **UIカタログを最初に確認** - ui-catalog-essentials.htmlで既存コンポーネント確認
2. **base.cssを確認** - 既存の定義・変数・ユーティリティクラスを確認
3. **ページ固有CSSを確認** - results.css/training.cssでページ固有スタイルを確認
4. **現状報告** - 既存実装の状態を把握・報告
5. **修正方針決定** - UIカタログのコンポーネントとbase.cssのクラスを優先使用
6. **⚠️ 注意**: system.cssは使用禁止（全てbase.cssに統合済み）
...
- ✅ **CSS変数の活用** (base.css定義の変数使用 - --space-4, --radius-lg等)
