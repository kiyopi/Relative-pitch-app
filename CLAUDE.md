...
### 🚧 **現在の作業状況（2025-11-10更新）**

#### **✅ Phase 1-2完了（基本機能・UX改善）**
- ✅ **Phase 1-1**: 音程判定調整完了（PitchPro v1.3.3で異常値0%達成）
- ✅ **Phase 1-2**: 設定ページ実装完了（データリセット・エクスポート・インポート機能）
- ✅ **Phase 2-1**: 総合評価の次のステップ実装完了（動的グレード計算・グレード別メッセージ）
- ✅ **Phase 2-2**: トレーニング記録ページ実装完了（統計情報・セッション一覧・グラフ表示）
- ✅ **PitchPro v1.3.3/v1.3.4統合**: パフォーマンス最適化・音域表示修正完了

**完了率**: Phase 1-2は100%完了、総合完了率は約45%

#### **📋 残タスク（Phase 3-5）**
- 📦 **Phase 3**: 分析機能の強化（デモページのみ存在、本番統合未実装）- 6-8時間
- 🔧 **Phase 4**: 下行モード追加（パラメータ準備のみ、機能未実装）- 13-19時間
- 🔄 **Phase 5**: UI・マーケティング調整（収益化モデル決定済み、UI調整必要）- 7-11時間

**残工数**: 26-38時間

#### **📚 詳細ドキュメント**
- **実装ロードマップ全体**: Serenaメモリ `PERM-final-implementation-status-20251110`
- **2025-11-10作業履歴**: `MODIFICATION_HISTORY_20251110.md`（v1.3.3/v1.3.4統合の完全記録）
- **Phase 1完了報告**: Serenaメモリ `PERM-implementation-roadmap-phase1-complete-20251110`
- **Phase 1-2完了報告**: Serenaメモリ `PERM-implementation-status-phase1-2-complete-20251110`

### 🌿 **現在のブランチ**
- **作業ブランチ**: `feature/modular-spa-architecture`
- **最新実装**:
  - PitchPro v1.3.3/v1.3.4統合完了
  - Phase 1-2完全実装完了
  - **音量テスト機能削除完了（v2.9.1）**
- **次期タスク**: Phase 5（UI・マーケティング調整）またはPhase 3（分析機能統合）

### 🔊 **2025-01-25 重要更新: 音量テスト機能削除と代替実装**

#### **削除の背景**
- **問題**: 準備ページとトレーニングページで基音再生音量が異なる環境差問題
- **原因**: 準備ページ（PitchShifter単独）とトレーニングページ（iOS Safari audioSession統合）の環境差
- **影響**: iPad miniで音割れ発生、ユーザー混乱を招く不整合

#### **実施した変更**
1. **準備ページから削除**:
   - ✅ 基音プレビュー再生ボタン（約250行削除）
   - ✅ 音量スライダー（-20～0dB調整機能）
   - ✅ `isPlayingBaseNote`フラグ・関連ロジック

2. **UI表示の修正**:
   - ✅ Step 2: 「音声・音量」→「音声テスト」
   - ✅ アイコン: `volume-2` → `mic`
   - ✅ 音域保存済み時: タイトルを「準備完了」に動的変更

3. **設定ページに代替実装**:
   - ✅ 5段階ティックスライダー（-20, -10, 0, +10, +20）
   - ✅ localStorage永続化（`pitchpro_base_note_volume_offset`）
   - ✅ トレーニングページで音量オフセット自動適用

#### **メリット**
- ✅ 環境差による音量不一致問題の根本解決
- ✅ プレビューなしでシンプル化（微調整の無限ループ防止）
- ✅ 準備フロー高速化（音声テスト削減）

#### **バックアップ**
- **ブランチ**: `backup/volume-test-feature`
- **保存内容**: 削除した音量テスト機能の完全実装

#### **関連ドキュメント**
- `/specifications/VOLUME_TEST_REMOVAL_HISTORY.md`: 詳細な経緯と実装説明
- `/Bolt/v2/pages/docs/PREPARATION_HTML_SPECIFICATION.md`: 準備ページ仕様書更新済み
- `/UI-Catalog/ui-catalog-controls.html`: ティックスライダーUI実装
...
## 🎯 **2025-11-19 重要更新: SPA開発経緯と総合アーキテクチャ仕様書完成**

### **背景**
MPA（マルチページアプリケーション）からSPAへの移行過程で3回の大きなリファクタリングを経験し、その過程と現在のアーキテクチャを包括的に文書化する必要が生じました。

### **完成した仕様書**

**`/specifications/SPA_DEVELOPMENT_JOURNEY_AND_ARCHITECTURE.md`**（1,700行超）

この仕様書は、SPA開発の完全な歴史的記録と技術リファレンスです。

#### **主要内容**

1. **3回のリファクタリング完全記録**
   - 第1回（2025-11-18）: リロード検出システム統一（1フラグ → 2フラグシステム）
   - 第2回（2025-01-17）: ページ初期化システム統一（4パターン → 1統一システム）
   - 第3回（2025-11-19）: NavigationManager統合徹底化（AudioDetector二重管理問題解決）

2. **システムファイル一覧**（34ファイル、約20,000行）
   - 8つの詳細テーブル（コントローラー、コアシステム、コンポーネント等）
   - ページレベル・システムレベルの依存関係マップ
   - ファイル命名規則、バージョン管理戦略
   - グローバル関数一覧、廃止予定ファイルリスト

3. **SPAの癖と対応パターン**
   - 2フラグシステムの理解と実装
   - Router統一初期化パターン
   - NavigationManager統一API
   - リロード・ブラウザバック・ダイレクトアクセス対応

4. **PitchPro Audio Processing**（専用開発ライブラリ）
   - リポジトリ: https://github.com/kiyopi/pitchpro-audio-processing
   - このアプリのために専用開発された音声処理ライブラリ
   - リアルタイム音程検出、音量測定、明瞭度判定を提供

### **主要な成果**

| 項目 | 達成内容 |
|------|---------|
| **リファクタリング回数** | 3回（すべて完全記録）|
...
- ✅ **新規ページ追加時**: Router統一初期化パターンの理解
- ✅ **ナビゲーション実装時**: NavigationManager統一APIの使用方法
- ✅ **マイク管理実装時**: AudioDetectorライフサイクル管理の理解
- ✅ **リロード対応実装時**: 2フラグシステムの実装パターン
- ✅ **SPAアーキテクチャ全体理解時**: 依存関係マップ、設計原則の参照
- ✅ **トラブルシューティング時**: SPAの癖と対応パターンの確認

### **関連ドキュメント**

- **NAVIGATION_HANDLING_SPECIFICATION.md** (v5.0.0): ナビゲーション・リソース管理の完全仕様
- **Serenaメモリ**: `PERM-reload-navigation-refactoring-design-20251118`（第1回リファクタリング）
- **Serenaメモリ**: `PERM-unified-page-initialization-design-20251117-1540`（第2回リファクタリング）
- **Serenaメモリ**: `PERM-microphone-permission-skip-analysis-20251119`（第3回最適化）

### **重要な設計原則**

1. **NavigationManager統一API**: すべてのナビゲーションは`NavigationManager.navigate()`を使用
2. **Router統一初期化**: すべてのページはpageConfigs設定レジストリで管理
3. **2フラグシステム**: 遷移証明フラグ（一時）+ ページ状態フラグ（永続）
4. **AudioDetectorライフサイクル**: トレーニングフロー全体で一元管理
5. **依存関係の明示**: Chart.js、DistributionChart、PitchProの待機管理
...
10. **🔄 モード別動線の理解必須（2025-11-12追加）**:
    - ランダム基音: `hasIndividualResults: true` → 毎セッション後ページ遷移 → sessionStorage必須
    - 連続・12音階: `hasIndividualResults: false` → ページ遷移なし → メモリ保持
    - **必ずlessonId単位でカウント**（モード単位ではない）
    - 詳細: `TRAINING_SPECIFICATION.md` 1.2節参照
11. **📅 日付記載プロトコル（仕様書・ドキュメント作成時）**:
...
**本質的価値（脳内ピアノ理論）:**
- **コミュニケーション能力向上**: 相手の声の音程を正確に捉え、感情・意図を読み取る
- **人間関係の改善**: 声のニュアンスから相手の気持ちを理解し、円滑なコミュニケーション
- **12音階モードの意義**: 半音単位の精密な認識 = 相手の感情の機微を読み取る能力

**副産物（音楽的効果）:**
- 歌唱力向上、音程精度改善、楽曲理解深化
- カラオケが上手くなる、楽器演奏が上達する

**教育的効果:**
- 体系的な音感学習、進捗の可視化、モチベーション維持

> **重要**: このアプリは「カラオケ上達」を主目的としていない。
> 真の価値は「脳内ピアノの精度向上 → 相手の声を正確に捉える → コミュニケーション能力向上」にある。
...
自分の脳内ピアノで認識
    ↓
認識精度が高い → 正確に理解 → 協和音（円滑な関係）
認識精度が低い → 誤解が生じる → 不協和音（対立・争い）
```
...
> **このアプリが目指す世界**: 音程を正確に認識し合える人々が増え、
> 理解し合える社会、争いの少ない世界の実現に貢献する。
...
### 🎯 **重要: 相対音感トレーニングの核心的理解（必読）**

**このアプリの独自性 - ユーザー音域ベースの12音階**

12音階モードは「C〜B」の固定音ではなく、**ユーザーの声の音域内で使える連続する12半音**を使用します。

**❌ よくある誤解:**
```
12音階 = C, C#, D, D#, E, F, F#, G, G#, A, A#, B（固定）
```

**✅ 正しい理解:**
```
12音階 = ユーザーの音域内で使える連続する12半音（可変）
```

**具体例:**
| ユーザー | 音域 | 12音階の実際の範囲 |
|---------|------|-------------------|
| Aさん | E3〜E5 | E3 → F3 → F#3 → G3 → ... → D#4 |
| Bさん | G3〜G5 | G3 → G#3 → A3 → A#3 → ... → F#4 |
| Cさん | C3〜C5 | C3 → C#3 → D3 → D#3 → ... → B3 |

**なぜこの設計か:**
- **相対音感**トレーニングなので、絶対音（C=ド）に依存しない
- ユーザーが自分の声で出せる範囲内で訓練することで、実践的な音感が身につく
- 各ユーザーの「基準点」が異なるのが相対音感の本質

**chromaticDirection（基音進行方向）の意味:**
- `ascending`: ユーザー音域内の12音を低→高の順で使用
- `descending`: ユーザー音域内の12音を高→低の順で使用
- `both`: 上昇12セッション + 下降12セッション = 24セッション

**scaleDirection（音階方向）との組み合わせ:**
- 各セッションで歌う8音（ドレミファソラシド）の方向
- `ascending`（上行）: ド→レ→ミ→...→高ド
- `descending`（下行）: ド→シ→ラ→...→低ド
