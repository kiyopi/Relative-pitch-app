...
| 状況 | 表示テキスト | 実装箇所 |
|------|-------------|----------|
| 低音待機 | `低音域測定: できるだけ低い声で「あー」と発声しましょう` | lines 707, 1516 |
| 低音測定中 | `そのまま声をキープしましょう` | line 1197 |
| 低音リトライ | `低音測定失敗 - 再測定します (${retryCount}/${maxRetries})` | line 1265 |
| 低音リトライ待機 | `低音域測定: できるだけ低い声で「あー」と発声しましょう` | lines 1307, 1623 |
| 低音スキップ | `低音測定をスキップします` | line 1286 |

#### **高音測定フェーズ**
| 状況 | 表示テキスト | 実装箇所 |
|------|-------------|----------|
| 高音待機 | `高音域測定: できるだけ高い声で「あー」と発声しましょう` | line 1516 |
| 高音測定中 | `そのまま声をキープしましょう` | line 1526 |
| 高音リトライ | `高音測定失敗 - 再測定します (${highRetryCount}/${maxRetries})` | line 1382 |
| 高音リトライ待機 | `高音域測定: できるだけ高い声で「あー」と発声しましょう` | lines 1332, 1627 |

#### **結果・エラーフェーズ**
| 状況 | 表示テキスト | 実装箇所 |
|------|-------------|----------|
| 部分結果 | `高音測定失敗 - 低音域のみの結果を表示` | line 1412 |
...
| 状況 | 表示テキスト | 実装箇所 |
|------|-------------|----------|
| 低音リトライ指導 | `より大きな声で低い音を出してください` | line 1266 |
| 低音リトライ詳細 | `よりきく、より低い音で歌ってください` | line 1308 |
| 高音リトライ指導 | `より大きな声で高い音を出してください` | line 1383 |
| 高音リトライ詳細 | `よりきく、より高い音で歌ってください` | line 1333 |

#### **進行・結果メッセージ**
| 状況 | 表示テキスト | 実装箇所 |
|------|-------------|----------|
| 高音進行 | `高音測定に進みます` | line 1287 |
| 部分結果利用可能 | `低音域の測定結果のみ利用可能です` | line 1413 |
...
**表示タイミング**: 致命的エラー・最大リトライ到達時
**実装箇所**: lines 1285, 1411, 1462, 1353
...
#### **2. 低音測定待機フェーズ**
```
Main Status: "低音域測定: できるだけ低い声で「あー」と発声しましょう"
...
#### **5. 高音測定フェーズ（成功時）**
```
Main Status: "高音域測定: できるだけ高い声で「あー」と発声しましょう"
Sub Info: "安定した声を認識したら自動で測定開始します"
...
| `updateBadgeForConfirmed()` | 完了チェックマーク | 1230, 1560, 1841 |
| `updateBadgeForFailure()` | 失敗警告表示 | 1264, 1381 |
| `updateBadgeForError()` | エラー表示 | 1285, 1411, 1462 |
