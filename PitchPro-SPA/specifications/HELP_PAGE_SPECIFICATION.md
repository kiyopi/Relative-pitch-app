...
2. [コンテンツ構成](#コンテンツ構成)
3. [技術仕様](#技術仕様)
4. [SPAページ遷移・リソース管理](#spaページ遷移リソース管理)
5. [UIデザイン仕様](#uiデザイン仕様)
6. [スタイル定義](#スタイル定義)
7. [実装時の注意事項](#実装時の注意事項)
...
// pageConfigs に追加（cleanup関数含む完全版は「SPAページ遷移・リソース管理」セクション参照）
'help': {
    init: 'initHelpPage',
    dependencies: [],
    preventDoubleInit: false,
    cleanup: async () => { /* AudioDetector破棄処理 */ }
}
```
...
ヘルプページは**静的コンテンツページ**に分類されます。

| 分類 | 該当ページ | 特徴 |
|------|-----------|------|
| **静的コンテンツ** | help, settings, records | マイク不使用、状態保持不要 |
| トレーニングフロー | preparation, training, result-session | マイク使用、NavigationManager管理 |
| 分析ページ | results-overview, premium-analysis | 依存関係あり（Chart.js等） |

### マイク制御・AudioDetector管理

ヘルプページはマイクを使用しませんが、前のページ（トレーニング等）からAudioDetectorが残っている可能性があります。`records`ページと同じパターンでクリーンアップを実装します。
...
1. **メモリ解放**: 不要なオーディオリソースを解放
2. **マイクアクセス解除**: マイクインジケーター（赤点）を消す
3. **クリーンな状態**: 次のトレーニング開始時に確実に再初期化

### フラグ管理

#### 2フラグシステムの概要

SPAでは以下の2つのフラグでページ遷移を管理します：

| フラグ | 用途 | 寿命 |
|--------|------|------|
| `normalTransition` | 正常遷移の証明 | 遷移時に設定、到達後に削除（一時的） |
| `xxxPageActive` | ページアクティブ状態 | ページ滞在中は保持（永続的） |

#### ヘルプページでのフラグ管理

ヘルプページは**静的コンテンツ**のため、以下の理由で保護フラグは不要です：

1. **マイク権限チェック不要**: 音声処理を行わない
2. **準備スキップ保護不要**: トレーニングフローに含まれない
3. **状態復元不要**: ステートレスなページ

```javascript
// ヘルプページでは以下のフラグは使用しない
// - preparationPageActive（準備ページ専用）
// - trainingPageActive（トレーニングページ専用）
// - normalTransitionToXxx（保護対象ページ専用）
```

#### 他ページへの遷移時の考慮

ヘルプページから他のページへ遷移する際は、`NavigationManager.navigate()`を使用します：

```javascript
// ナビゲーションボタンの実装例
<button class="nav-button" onclick="NavigationManager.navigate('#home')" title="ホーム">
...
ヘルプページをリロードした場合：
1. Router統一初期化システムにより`initHelpPage()`が呼ばれる
2. FAQアコーディオンの状態はリセット（デフォルト：すべて閉じた状態）
3. Lucideアイコンは再初期化される
...
1. index.htmlが読み込まれる
2. router.jsがハッシュを検出
3. `pages/help.html`テンプレートが読み込まれる
4. `initHelpPage()`が実行される

**特別な保護は不要**: 静的コンテンツのため、どのような経路でアクセスしても正常に表示される
...
|------|------|------|
| Chart.js | ❌ | グラフ表示なし |
| DistributionChart | ❌ | 分布表示なし |
| PitchPro | ❌ | 音声処理なし |
| NavigationManager | ⚠️ | cleanup時のみ参照（オプショナル） |
| Lucide | ✅ | アイコン表示に必須 |

### pageConfigs完全定義

```javascript
// router.js に追加する完全な設定
'help': {
    init: 'initHelpPage',
    dependencies: [],
    preventDoubleInit: false,
...
- [ ] router.js: pageConfigsにhelp設定追加（cleanup関数含む）
- [ ] index.html: help.css読み込み追加
- [ ] index.html: ナビゲーションボタン追加
- [ ] index.html: コントローラー読み込み追加

### SPA遷移・リソース管理確認
- [ ] cleanup関数がAudioDetectorを正しく破棄確認
- [ ] トレーニング中断→ヘルプ→マイクインジケーター消灯確認
- [ ] ヘルプページ直接アクセス（`#help`）動作確認
- [ ] ヘルプページリロード時の正常表示確認
- [ ] ヘルプ→トレーニング開始時のマイク再初期化確認
