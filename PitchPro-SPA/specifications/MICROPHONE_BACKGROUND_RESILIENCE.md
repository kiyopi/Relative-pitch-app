...
**作成日**: 2025-11-09
**最終更新日**: 2025-11-27
**ステータス**: ✅ v4.5.0でトレーニングページ事前チェック追加、v1.2.6でidleTimeout短縮
...
### v3.1.0 (2025-11-27)
**PitchPro idleTimeout短縮（10分→3分）**

PitchProライブラリのマイクアイドルタイムアウトを10分から3分に短縮しました。

**変更内容**:
- **変更前**: idleTimeout = 600,000ms（10分）
- **変更後**: idleTimeout = 180,000ms（3分）

**設定場所**: `/PitchPro-SPA/js/core/pitchpro-config.js` (v1.2.6)
```javascript
lifecycle: {
...
**idleTimeoutの動作**:
- ユーザーが指定時間マイクに音声入力しない場合、`MicrophoneLifecycleManager`が自動でマイクを停止
- 検出ログ: `😴 [MicrophoneLifecycleManager] User idle detected`
- 次回マイク使用時にgetUserMedia()が再度呼ばれる

**影響分析**:

| シナリオ | 最大所要時間 | 3分超過? | 影響 |
|---------|-------------|---------|------|
| 音域テスト（低音・高音測定） | 30秒程度 | ❌ | なし |
| トレーニング1セッション | 約8秒 | ❌ | なし |
| ランダム基音8セッション連続 | 約64秒 | ❌ | なし |
| 連続チャレンジ12セッション | 約2分 | ❌ | なし |
| 12音階24セッション連続 | 約4分 | ⚠️ | 後述 |

**12音階モードについて**:
- 24セッション連続は約4分だが、**ドレミガイド中は常に音声入力がある**
- 音声入力があるとアイドルタイマーがリセットされるため、実質的に問題なし

**潜在的なリスクシナリオ**:
1. **準備画面で3分以上放置**: マイクテスト完了後、トレーニング開始ボタンを押さずに放置 → 次回マイク再初期化が必要
2. **結果画面で3分以上放置**: ランダム基音モードの各セッション後の結果画面で長時間放置 → 次回マイク再初期化が必要

**結論**: 通常の使用フローでは影響なし。放置シナリオでもgetUserMedia()が再度呼ばれるだけで正常動作。
