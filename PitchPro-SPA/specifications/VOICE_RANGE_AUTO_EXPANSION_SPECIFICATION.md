...
**具体例**:
```
❌ 修正前: C3 → C3 → D3 → E3 ... (連続重複発生)
✅ 修正後: C3 → D3 → E3 → F3 ... (連続重複なし)
```
...
| `trainingController.js` | 基音自動拡張・基音選定 | `getAvailableNotes()`, `selectAllBaseNotesForMode()`, `selectContinuousMode()` | v3.0.0 (拡張), v2.0.0 (連続) |
...
**将来の拡張案** (Phase 2):
- オクターブ跳躍の導入（例: C3→E4→G2→A3）
- 音域2.5オクターブ以上の場合に有効化
- 音程間隔分析との整合性を保持

**注意**: オクターブ跳躍を導入すると、音程間隔別精度分析（2度±18¢、3度±24¢等）が複雑化するため、Phase 2として慎重に検討する。
...
| バージョン | 日付 | 変更内容 |
|------------|------|----------|
| 1.1.0 | 2025-11-10 | 連続チャレンジモード連続重複防止機能追加・セクション8「モード別基音選定システム」追加 |
