...
**v1.5.0 更新内容**:
- 脳内ピアノ理論の完全統合（フーリエ解析理論 + 左脳・両脳処理メカニズム）
- 左脳処理音（C-F#）と両脳処理音（G-B）の詳細説明追加
- トレーニングへの応用方法の明確化
- 将来の拡張計画の記載（交互発声モード・苦手克服モード）
...
- 音量調整ガイド（適切な音量範囲の表示）
- **音域テスト指示エリア**:
  - 「低音テスト: できるだけ低い声で「あー」と発声してください」
  - 「高音テスト: できるだけ高い声で「あー」と発声してください」
  - テスト状況のプログレス表示
- **音域結果表示**:
  - 検出された音域範囲（例：B2 - D5）
  - オクターブ数（例：2.3オクターブ）
...
5. **結果表示**
   ...
   - 合格音数（例: 6/8音）
   - 各音階の精度表示（セント単位）
   - ランダムモードのみ1セッションごとに4段階評価で8セッション終了時に総合評価表示
   - 総合評価は6段階
...
#### 初級者向けの特徴
- **集中時間**: 1セッションのみで短時間
- **基音の数**: 音域テストデータをもとにしてオクターブの8音（半音なし）
...
### 3.3 音域対応12音階モード（中級+）※v1.5.0実装予定
**URL**: `/training?mode=12tone-adaptive`

#### モードの目的と特徴
- **対象**: ランダム基音・連続チャレンジモードを習得した中級者
- **目的**: ユーザーの音域内で使える音階を順次練習し、12音階完全版への準備を行う
- **学習効果**: クロマチック（半音階）の相対的な音感の差を段階的に習得
- **最低音域要件**: **1.0オクターブ以上**
...
#### セッション構成
```
セッション数: 利用可能な音数に応じて動的決定（例：9音なら9セッション）
基音選択: 音域内で使える音を順次使用
評価: 連続チャレンジと同様（個別評価なし、総合評価のみ）
```

#### 画面表示例
```
モード名: 音域対応12音階モード
現在の利用可能音数: 9/12音 (C3～A3)
セッション数: 9セッション
```

#### 完了後のメッセージ
```
おめでとうございます！9音でクリアしました。

完全な12音階モードに挑戦するには、音域を広げるトレーニングをお勧めします。
目標音域: 2.0オクターブ以上

[音域トレーニングガイドを見る] [ホームに戻る]
```

---

### 3.4 12音階モード完全版（上級）※v1.6.0実装予定
**URL**: `/training?mode=12tone`

#### モードの目的と特徴
- **対象**: 音域対応版を習得し、2.0オクターブ以上の音域を持つ上級者
- **目的**: クロマチック12音すべてに対して完璧な相対音感を持つ（最終目標）
- **学習効果**: どんな調性の楽曲でも正確に歌える能力
- **最低音域要件**: **2.0オクターブ必須**
...
```
セッション数: 12セッション固定
基音選択: クロマチック12音すべてを順次使用
評価: 総合評価のみ
```

#### 基本仕様（v1.6.0）
- **基音選択**: クロマチック12音すべてを順次使用（C、C#、D...B）
- **ドレミガイド**: 上昇のみ（ド→レ→ミ→ファ→ソ→ラ→シ→ド）
- **判定基準**: より厳密（±30セント以内）
- **総合評価**: 12セッション完了後に総合評価画面へ遷移

#### 将来の拡張（v2.0以降予定）
上昇・下降モード、音域調整機能などの追加機能は将来のバージョンで実装予定です。
詳細は`FUTURE_FEATURES.md`を参照してください。
