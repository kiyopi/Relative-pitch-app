...
### 7.1 データエクスポート/インポート（実装済み）

#### エクスポート機能
- **実装場所**: `data-manager.js` - `exportData()`
- **ファイル名形式**: `8va-pitchtraining-data-[タイムスタンプ].json`
  - 例: `8va-pitchtraining-data-1732876543210.json`
- **出力形式**: JSON
- **含まれるデータ**:
  - トレーニング記録（全セッション履歴）
  - 音域設定データ
...
#### インポート機能
- **実装場所**: `data-manager.js` - `importData()`
- **対応形式**: 上記エクスポートファイル（JSON）
- **動作**: 既存データを上書き
- **用途**: デバイス移行、バックアップ復元

#### デバイス移行手順
1. 移行元: 設定ページ → データエクスポート
2. ファイル転送: クラウドストレージ、AirDrop、メール等
3. 移行先: 設定ページ → データインポート
4. 推奨: 新デバイスで音域テスト再実行（マイク特性差のため）
