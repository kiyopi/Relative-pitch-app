...
**変更履歴**:
- v1.1.0 (2025-11-12): PitchPro設定強化（低音域オクターブ誤検出対応）
  - `minFrequency`: 80Hz → 60Hz（F3の1オクターブ下 87.3Hzもカバー）
  - `harmonicConfig`追加: 倍音補正の詳細設定
    - `enabled`: true
    - `historyWindow`: 2000ms（2秒の履歴を使用）
    - `minHistorySize`: 8（最小8サンプル）
  - **修正ファイル**: `/PitchPro-SPA/js/core/pitchpro-config.js`
- v1.0.0 (2025-10-27): 初版作成
